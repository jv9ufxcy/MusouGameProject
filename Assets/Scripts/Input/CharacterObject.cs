...
public class CharacterObject : MonoBehaviour, IHittable
{
    ...
    {
        draw.transform.LookAt(Camera.main.transform);
        draw.transform.rotation = Quaternion.Euler(0f, transform.rotation.eulerAngles.y, 0f);
    }
    ...
    void UpdateAnimator()
    {
        ...
        aniMoveSpeed = Vector3.SqrMagnitude(latSpeed) * 30;
    ...
    void FaceStick(float _rate)//flip hitbox
    {
        if (leftStick != Vector2.zero)
            facingDir = leftStick;
